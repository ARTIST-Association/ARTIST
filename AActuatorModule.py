...
class ActuatorModule(AModule):
    ...
    def _stepsToAngles(self, actuator_pos: torch.Tensor):
        ...
        angles = self._phi_0() + delta_phi if self.CLOCKWISE else self._phi_0() - delta_phi
        return angles
    ...
    def _anglesToSteps(self, angles : torch.Tensor):
        ...
        stroke_length = torch.sqrt( self._actuator_offset() ** 2  + self._joint_radius() ** 2 - calc_step_1)
        actuator_steps = (stroke_length - self._initial_stroke_length()) * self._increment()
