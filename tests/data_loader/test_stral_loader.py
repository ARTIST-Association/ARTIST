...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                torch.tensor([[-0.807500004768,  0.642499983311,  0.040198374540],
        [ 0.807500004768,  0.642499983311,  0.040198374540],
        [-0.807500004768, -0.642499983311,  0.040198374540],
        [ 0.807500004768, -0.642499983311,  0.040198374540]]),
                torch.tensor([[ 8.024845123291e-01,  0.000000000000e+00,  4.984567407519e-03],
...
def test_extract_paint_heliostats(
    ...
    torch.testing.assert_close(
        extracted_list[2][3][0], 
        expected[2].to(device)
    )
    torch.testing.assert_close(
        extracted_list[3][1][2], 
        expected[3].to(device)
    )
