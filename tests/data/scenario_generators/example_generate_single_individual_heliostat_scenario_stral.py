...
heliostat_kinematic_offsets = KinematicOffsets(
    kinematic_initial_orientation_offset_e=torch.tensor(
        torch.tensor(torch.pi / 2, device=device), device=device
    )
)
...
heliostat_kinematic_config = KinematicConfig(
    kinematic_type=config_dictionary.rigid_body_key,
    kinematic_initial_orientation_offsets=heliostat_kinematic_offsets,
)
...
actuator_heliostat1_list = [actuator1_heliostat, actuator2_heliostat]
heliostat_actuators_config = ActuatorListConfig(actuator_list=actuator_heliostat1_list)
