...
def find_viable_heliostats(
    ...
    all_heliostats = (
        ...
        if d.is_dir() 
        and heliostat_name_pattern.match(d.name)
        and (heliostat_list is None or d.name in heliostat_list)
    )
            ...
            try:
                with calibration_file_path.open("r") as f:
                    ...
                    if paint_mappings.UTIS_KEY in focal_spot_data:
                        ...
                        if kinematic_reconstruction_flux_image_path.exists() and surface_reconstruction_flux_image_path.exists():
                            valid_calibration_files.append(calibration_file_path)
                            flux_images_kinematic_reconstruction.append(kinematic_reconstruction_flux_image_path)
                            flux_images_surface_reconstruction.append(surface_reconstruction_flux_image_path)
        ...
        if len(valid_calibration_files) >= minimum_number_of_measurements:
            found_heliostats.append(
                (
                    ...
                    flux_images_kinematic_reconstruction[:minimum_number_of_measurements],
...
if __name__ == "__main__":
    ...
    number_of_heliostats_in_baseline_default = config.get(
        "number_of_heliostats_in_baseline", 100
    )
    number_of_heliostats_in_full_field_default = config.get(
        "number_of_heliostats_in_full_field", 2000
    )
    kinematic_reconstruction_image_type_default = config.get("kinematic_reconstruction_image_type", "flux")
    surface_reconstruction_image_type_default = config.get("surface_reconstruction_image_type", "flux-centered")
    ...
    parser.add_argument(
        "--number_of_heliostats_in_baseline",
        type=int,
        help="Maximum number of heliostats to include.",
        default=number_of_heliostats_in_baseline_default,
    )
    parser.add_argument(
        "--number_of_heliostats_in_full_field",
        type=int,
        help="Maximum number of heliostats to include.",
        default=number_of_heliostats_in_full_field_default,
    )
    parser.add_argument(
    ...
    for case, number_of_heliostats in zip(["baseline", "full_field"], [args.number_of_heliostats_in_baseline, args.number_of_heliostats_in_full_field]):
        
        if case == "baseline": 
            heliostat_list=args.heliostat_list
        else:
            heliostat_list = None
        ...
        heliostat_data_list = find_viable_heliostats(
            ...
            maximum_number_of_heliostats=number_of_heliostats,
        ...
        for (
            ...
            print(
                f"- {heliostat_name}: {len(calibration_paths)} calibrations, {len(kinematic_reconstruction_flux_image_path)} {args.kinematic_reconstruction_image_type} images and {len(surface_reconstruction_flux_image_path)} {args.surface_reconstruction_image_type} images"
            )
        ...
        serializable_data = [
            {
                ...
                "kinematic_reconstruction_flux_images": [str(flux_path) for flux_path in kinematic_reconstruction_flux_paths],
                "surface_reconstruction_flux_images": [str(flux_path) for flux_path in surface_reconstruction_flux_image_path],
