...
def calibration_path_to_sun_and_tower(paths, target_area_names, device):  # noqa: D103
    ...
    sun_positions = utils.convert_3d_points_to_4d_format(paint_loader.azimuth_elevation_to_enu(azimuth=torch.tensor(azimuths), elevation=torch.tensor(elevations), device=device), device=device)
...
def plot_surface_points(points, name):
    ...
    for facet in points:
        x, y, z = facet[:, 0].cpu().detach(), facet[:, 1].cpu().detach(), facet[:, 2].cpu().detach()
...
def plot_normal_angle_map(surface_points, surface_normals, reference_direction, name):
    ...
    normals = (surface_normals[..., :3] / torch.linalg.norm(surface_normals[..., :3], axis=-1, keepdims=True)).cpu().detach()
    ref = (reference_direction[..., :3] / torch.linalg.norm(reference_direction[..., :3])).cpu().detach()

    for facet_points, facet_normals in zip(surface_points.cpu().detach(), normals):
        x, y, z = facet_points[:, 0].cpu().detach(), facet_points[:, 1].cpu().detach(), facet_points[:, 2].cpu().detach()
        
        sc0 = axes[0].scatter(x, y, c=z, cmap='viridis', s=5)
        ...
        sc1 = axes[1].scatter(x, y, c=angles, cmap='plasma', s=5)
