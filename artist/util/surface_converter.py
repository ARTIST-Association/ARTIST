...
class SurfaceConverter:
    ...
    def _create_nurbs_surface(self, surface_points: torch.Tensor, device: torch.device):
    ...
    def _fitting_from_points_or_normals(
        self,
        ...
        nurbs_surface = self._create_nurbs_surface(surface_points, device)
    ...
    def _create_list_of_facet_configs_from_fitting(
        self,
        ...
        if not isinstance(self.surface_generation_config, FitConfig):
            raise TypeError("surface_generation_config must be an instance of FitConfig to fit a NURBS surface.")
        ...
        for i in range(surface_points_with_facets.shape[0]):
            ...
            nurbs_surface = self._fitting_from_points_or_normals(
    ...
    def fit_nurbs_surface_from_points( # @MarleneBusch should this function also use _generate_surface_config?
        self,
        surface_points: torch.Tensor,
        surface_normals: torch.Tensor,
    ) -> NURBSSurface:
        ...
        control_points_shape = (self.nurbs_config.number_control_points_e, self.nurbs_config.number_control_points_n)
        ...
        origin_offsets_e = torch.linspace(
            -width_of_nurbs / 2,
            width_of_nurbs / 2,
            self.nurbs_config.number_control_points_e,
            device=device,
        )
        origin_offsets_n = torch.linspace(
            -height_of_nurbs / 2,
            height_of_nurbs / 2,
            self.nurbs_config.number_control_points_n,
            device=device,
        )
        ...
        optimizer = torch.optim.Adam([control_points], lr=self.surface_generation_config.initial_learning_rate)
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode="min",
            factor=0.2,
        ...
        loss = torch.inf
        epoch = 0
        while loss > self.surface_generation_config.tolerance and epoch <= self.surface_generation_config.max_epoch:
            points, normals = nurbs_surface.calculate_surface_points_and_normals(
                device=device
            )
            ...
            if self.surface_generation_config.conversion_method == config_dictionary.convert_nurbs_from_points:
                loss = (points - surface_points).abs().mean()
            elif self.surface_generation_config.conversion_method == config_dictionary.convert_nurbs_from_normals:
                loss = (normals - surface_normals).abs().mean()
            else:
                raise NotImplementedError(
                    f"Conversion method {self.surface_generation_config.conversion_method} not yet implemented!"
                )
            loss.backward()
    ...
    def fit_surface_config_from_stral(
        ...
        surface_config = self._create_list_of_facet_configs_from_fitting(
    ...
    def fit_surface_config_from_paint(
        self,
    ...
    def generate_nurbs_surface_analyticaly(self) -> NURBSSurface:
            # ------------------------------------------------------------
            # 1) Guard: analytic-only instances can’t fit
            # ------------------------------------------------------------
            if not isinstance(self.surface_generation_config, AnalyticalConfig):
                raise RuntimeError(
                    "Cannot fit: converter was not created in analytic mode."
                )
