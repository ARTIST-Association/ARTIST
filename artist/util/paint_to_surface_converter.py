    ...
    def generate_surface_config_from_paint(
        ...
        with open(self.heliostat_file_path, 'r') as file:
            ...
            for facet in range(number_of_facets):
                facet_translation_vectors[facet, :] = torch.tensor(heliostat_dict["facet_properties"]["facets"][facet]["translation_vector"])
                canting_e[facet, :] = torch.tensor(heliostat_dict["facet_properties"]["facets"][facet]["canting_e"])
        ...
        with h5py.File(self.deflectometry_file_path, "r") as file:
            ...
            for f in range(number_of_facets):
                ...
                points_data = torch.tensor(numpy.array(file[f"facet{f+1}"]["surface_points"]))
                normals_data = torch.tensor(numpy.array(file[f"facet{f+1}"]["surface_normals"]))
