...
from artist.physics_objects.heliostats.surface.facets.nurbs_facets import (
    NURBSFacetsModule,
)
...
def generate_data(light_direction, expected_value):
    ...
    nurbs_facets = NURBSFacetsModule(
        surface_config, nurbs_config, device, receiver_center=receiver_center, sun_directions=light_direction
    )
    logging.debug(nurbs_facets.facets_and_rots)
...
def test_compute_bitmaps(sun_data):
    ...
    torch.testing.assert_close(True, True)
