...
from artist.physics_objects.heliostats.surface.facets.canting import ActiveCanting, CantingAlgorithm
import artist.physics_objects.heliostats.surface.facets.facets_copied as facets
from artist.physics_objects.heliostats.surface.facets.facets_copied import AbstractFacets, AlignedFacets, Facets
import artist.util.utils as utils
...
def real_heliostat(
    ...
    (
        ...
        cfg.FILENAME,
        struct.Struct(cfg.CONCENTRATORHEADER_STRUCT_FMT),
        struct.Struct(cfg.FACETHEADER_STRUCT_FMT),
        struct.Struct(cfg.RAY_STRUCT_FMT),
        cfg.VERBOSE,
    )
    ...
    if cfg.ZS_PATH:
        ...
        integrated = torch.tensor(integrated)[:,:,[1,0,2]]
        integrated[:,:,1] *= -1
    ...
    # print( facet_spans_e)
    # ar = np.array(facet_spans_e)
    # facet_spans_e = list(-ar)
    # print(facet_spans_e)
...
class Heliostat(AbstractHeliostat):
    ...
    def __init__(
        ...
        self._to_optimize: List[str] = self.cfg.TO_OPTIMIZE_NEW
    ...
    def _get_rotation_offset(self, cfg: CfgNode) -> torch.Tensor:
        rotation_offset: float = cfg.ROTATION_OFFSET_ANGLE
        return th.deg2rad(torch.tensor(
            rotation_offset,
            dtype=self.aim_point.dtype,
            device=self.device,
        ))
    ...
    def load(
        ...
        self.rotation_offset = self._get_rotation_offset(self.cfg)
    ...
    def _fixed_dict(self) -> Dict[str, Any]:
        ...
        data = {
            ...
            'rotation_offset_angle_rad': self.rotation_offset.clone(),
