...
from artist.util import runtime_log, track_runtime
...
def create_blocking_primitives(
    ...
    distances_to_surface_points = torch.abs(surface_points_2d[:,:,None,:] - min_max_values[:, None, :, :])
    ...
    corners = blocking_heliostats_active_surface_points[surface_indices, corner_points_indices]

    spans = torch.zeros((number_of_surfaces, 2, 4), device=device)
    spans[:, 0] = corners[:, 1] - corners[:, 0]
    spans[:, 1] = corners[:, 3] - corners[:, 0]

    plane_normals = torch.nn.functional.normalize(torch.cross(spans[:, 0, :3], spans[:, 1, :3], dim=-1), dim=-1)
...
def compute_soft_ray_blocking(
    ...
    distances_to_blocking_planes = torch.sum((corner_0 - ray_origins) * blocking_primitives_normals, dim=-1) / (denominator + epsilon)
    blocking_planes_in_front_of_heliostats = torch.sigmoid(softness * (distances_to_blocking_planes - 1e-3)) 

    intersection_points = ray_origins + distances_to_blocking_planes[..., None] * ray_directions
    ...
    det = span_u_squared_norm * span_v_squared_norm - span_uv_dot * span_uv_dot + epsilon
    u_coordinate_on_plane = (offset_projection_u * span_v_squared_norm - offset_projection_v * span_uv_dot) / det
    v_coordinate_on_plane = (offset_projection_v * span_u_squared_norm - offset_projection_u * span_uv_dot) / det
    ...
    blocking_planes_in_front_of_target = torch.sigmoid(softness * (distances_to_target.unsqueeze(-1) - distances_to_blocking_planes))
    
    blocking_mask_per_plane = blocking_within_plane * blocking_planes_in_front_of_heliostats * blocking_planes_in_front_of_target
...
def expand_bits(integers: torch.Tensor) -> torch.Tensor:
    ...
    expanded_integers = (expanded_integers | (expanded_integers << 16)) & 0b111000000000000001111111
    expanded_integers = (expanded_integers | (expanded_integers << 8)) &  0b111000001111000000001111
    expanded_integers = (expanded_integers | (expanded_integers << 4)) &  0b110000110000110000110011
    expanded_integers = (expanded_integers | (expanded_integers << 2)) &  0b1001001001001001001001001001
...
def morton_codes(
    ...
    xx = expand_bits(xi)             # Spread 10 bits into 30 bits with 2 zero bits between each bit.
...
def most_significant_differing_bit(
    ...
    most_significant_bits = torch.full_like(differing_bits, -1, dtype=torch.int64, device=device)
...
def longest_common_prefix(
    ...
    most_significant_differing_bits = most_significant_differing_bit(differing_bits, device=device)
    longest_common_prefixes = torch.where(
        differing_bits == 0, 
        torch.full_like(most_significant_differing_bits, total_bits, dtype=torch.int64, device=device),
...
def build_linear_bounding_volume_hierarchies(
    ...
    mask_out_of_bounds = (neighbor_indices >= 0) & (neighbor_indices < number_of_blocking_primitives)
    neighbor_indices = torch.clamp(neighbor_indices, 0, number_of_blocking_primitives - 1)

    delta_min = longest_common_prefix(codes=sorted_codes, i=blocker_ids, j=neighbor_indices, device=device)
    delta_min = torch.where(
        mask_out_of_bounds, 
        delta_min, 
        torch.full_like(delta_min, -1, device=device)
    )

    max = math.ceil(math.log2(number_of_blocking_primitives)) if number_of_blocking_primitives > 1 else 1
    farthest_expansion = torch.zeros(number_of_blocking_primitives, dtype=torch.int64, device=device)

    for k in range(0, max + 1):
        step = 1 << k
        candidate_indices = blocker_ids + direction_to_similar_codes * (farthest_expansion + step)
        mask_out_of_bounds_candidates = (candidate_indices >= 0) & (candidate_indices < number_of_blocking_primitives)
        candidate_indices = torch.clamp(candidate_indices, 0, number_of_blocking_primitives - 1)
        candidates_lcp = longest_common_prefix(sorted_codes, blocker_ids, candidate_indices, device=device)
        mask = mask_out_of_bounds_candidates & (candidates_lcp > delta_min)
        farthest_expansion = torch.where(mask, farthest_expansion + step, farthest_expansion)
    ...
    if max_span < 1:
        ...
        for k in range(max_k, -1, -1):
            ...
            candidates_indices = torch.clamp(candidate_indices, 0, number_of_blocking_primitives - 1)
            candidates_incremented_indices = torch.clamp(candidate_indices + 1, 0, number_of_blocking_primitives - 1)
            candidates_lcp = longest_common_prefix(codes=sorted_codes, i=min_index, j=candidates_indices, device=device)
            candidates_incremented_lcp = longest_common_prefix(codes=sorted_codes, i=min_index, j=candidates_incremented_indices, device=device)
    ...
    internal_nodes_indices = torch.arange(0, internal_count, dtype=torch.int64, device=device)
    
    # Map the original primitive index via the sorted_primitive_indices.
    aabb_min[leaf_offset:leaf_offset + number_of_blocking_primitives] = primitive_mins[sorted_primitive_indices]
    aabb_max[leaf_offset:leaf_offset + number_of_blocking_primitives] = primitive_maxs[sorted_primitive_indices]
    is_leaf[leaf_offset:leaf_offset + number_of_blocking_primitives] = True
    primitive_index[leaf_offset:leaf_offset + number_of_blocking_primitives] = sorted_primitive_indices.to(torch.int32)

    # left child node id corresponds to range [first[i], split[i]].
    # right child node id corresponds to range [split[i]+1, last[i]].
    left_child_nodes = range_to_node_id(start_indices=min_index[:internal_count], end_indices=split[:internal_count], leaf_offset=leaf_offset)
    right_child_nodes = range_to_node_id(start_indices=split[:internal_count] + 1, end_indices=max_index[:internal_count], leaf_offset=leaf_offset)

    # Detect cycles and replace by leaves.
    left_child_ids = torch.where(left_child_nodes == internal_nodes_indices, leaf_offset + min_index[:internal_count], left_child_nodes)
    right_child_ids  = torch.where(right_child_nodes == internal_nodes_indices, leaf_offset + max_index[:internal_count], right_child_nodes)
    ...
    nodes_with_complete_aabb = torch.zeros(internal_count, dtype=torch.bool, device=device)
    ...
    while not nodes_with_complete_aabb.all() and rounds < internal_count:
        ...
        internal_mask = left_is_internal & (left_internal >= 0) & (left_internal < internal_count)
        left_done = torch.ones_like(left_is_internal, dtype=torch.bool, device=device)
        left_done[internal_mask] = nodes_with_complete_aabb[left_internal[internal_mask]]

        right_is_internal = right_internal < leaf_offset
        internal_mask = right_is_internal & (right_internal >= 0) & (right_internal < internal_count)
        right_done = torch.ones_like(right_is_internal, dtype=torch.bool, device=device)
        right_done[internal_mask] = nodes_with_complete_aabb[right_internal[internal_mask]]

        nodes_to_be_computed_next = (~nodes_with_complete_aabb) & left_done & right_done
        if not nodes_to_be_computed_next.any():
            break

        next_nodes_indices = torch.nonzero(nodes_to_be_computed_next, as_tuple=True)[0].to(device)
        l = left_internal[next_nodes_indices]
        r = right_internal[next_nodes_indices]

        mins = torch.minimum(aabb_min[l], aabb_min[r])
        maxs = torch.maximum(aabb_max[l], aabb_max[r])
    ...
    if not nodes_with_complete_aabb.all():
        warnings.warn(
            f"BVH AABB fallback (slow calculation) triggered: {incomplete.numel()} internal nodes did not receive AABBs via DAG propagation.", RuntimeWarning)
        ...
        for node in incomplete.tolist():
            ...
            leaf_nodes_slice = torch.arange(min, max + 1, device=device, dtype=torch.int64) + leaf_offset
...
def blocking_filter_lbvh(
    ...
    lbvh = build_linear_bounding_volume_hierarchies(blocking_primitives_corners=blocking_primitives_corners, device=device)
    ...
    mask_hits_per_ray = torch.zeros((total_number_of_rays, number_of_primitives), dtype=torch.bool, device=device)
    ...
    while active_rays.numel() > 0:
        ...
        mask_hit = (exit_distance_to_aabb >= entry_distance_to_aabb) & (exit_distance_to_aabb > 1e-6)
        ...
        if mask_hit.any():
            ...
            if (~leaf_mask).any():
                ...
                if ((index_for_left_children >= max_stack_size) & (index_for_left_children != -1)).any() or \
                   ((index_for_right_children >= max_stack_size) & (index_for_right_children != -1)).any():
                    raise RuntimeError("Stack overflow in LBVH traversal, max_stack too small.")
    ...
    filtered_blocking_primitive_indices = torch.nonzero(non_self.any(dim=0), as_tuple=True)[0]
