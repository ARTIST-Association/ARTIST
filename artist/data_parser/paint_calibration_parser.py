...
class PaintCalibrationDataParser(CalibrationDataParser):
    ...
    def _parse_calibration_data(
        ...
        focal_spots_global = torch.empty((total_samples, 3), dtype=torch.float64, device=device)
        ...
        for name in heliostat_names:
            for (
                ...
                focal_spots_global[index] = torch.tensor(focal_spot, dtype=torch.float64, device=device)
