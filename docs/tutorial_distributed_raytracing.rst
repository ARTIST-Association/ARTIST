...
It is still possible to set a specific default target area index and a default incident ray direction later. If these
are not provided, all heliostats are assigned to the first target area found in the scenario with an incident ray
direction of "north", i.e., the light source position is directly in the south.
...
Consider an example scenario of two heliostat groups with two heliostats each in a distributed environment with three
processes:
...
.. list-table:: Bitmaps per target area (on the ``multi_focus_tower``) after nested ``all_reduce``.
   :widths: 33 33 33
   :header-rows: 0
...
.. list-table:: Bitmaps per target area (on the ``multi_focus_tower``) after final ``all_reduce``.
   :widths: 33 33 33
   :header-rows: 0
