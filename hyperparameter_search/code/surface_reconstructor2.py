            ...
            if active_heliostats_mask.sum() > 0:
                ...
                while loss > self.tolerance and epoch <= self.max_epoch:
                    ...
                    if epoch in [0, 10, 50, 100, 300, 500, 1000, 2000, 3000]:
                        ...
                        os.makedirs(
                            (
                                pathlib.Path("/home/hgf_dlr/hgf_zas3427/artist-data")
                                / folder_name
                            ),
                            exist_ok=True,
                        )
                        ...
                        with open(
                            (
                                pathlib.Path(
                                    f"/home/hgf_dlr/hgf_zas3427/artist-data/{folder_name}"
                                )
                                / "loss.txt"
                            ),
                            "a",
                        ...
                        with torch.no_grad():
                            for i in range(heliostat_group.number_of_heliostats):
                                ...
                                name1 = (
                                    pathlib.Path(
                                        "/home/hgf_dlr/hgf_zas3427/artist-data"
                                    )
                                    / folder_name
                                    / f"points_and_normals_h_{i}_epoch_{epoch}"
                                )
                            ...
                            validation_normalized_measured_flux_distributions = flux_distribution_loader.load_flux_from_png(
                            ...
                            references = torch.zeros_like(
                                validation_bitmaps_per_heliostat
                            )
                            references[
                                : validation_normalized_measured_flux_distributions.shape[
                                    0
                                ]
