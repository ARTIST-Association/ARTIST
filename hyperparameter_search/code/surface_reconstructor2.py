    ...
    def reconstruct_surfaces(
        ...
        if self.heliostats_mask.sum() > 0:
            ...
            while loss > self.tolerance and epoch <= self.max_epoch:
                ...
                if epoch in [10, 50, 100, 300, 500, 1000, 2000, 3000]:

                    name1 = pathlib.Path("/workVERLEIHNIX/mb/ARTIST/hyperparameter_search/nrays_220_sp_90/heliostats_2") / f"points_and_normals_epoch_{epoch+1}_cp_{self.heliostat_group.nurbs_control_points.shape[2]}"
                    name2 = pathlib.Path("/workVERLEIHNIX/mb/ARTIST/hyperparameter_search/nrays_220_sp_90/heliostats_2") / f"fluxes_epoch_{epoch+1}_cp_{self.heliostat_group.nurbs_control_points.shape[2]}"
                    ...
                    with torch.no_grad():
                        ...
                        helper.plot_multiple_fluxes(
                            ...
                            references=self.normalized_measured_flux_distributions,
                            name=name2
                        )
