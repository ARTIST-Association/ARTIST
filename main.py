...
def load_heliostat(cfg, device):
    ...
    if cfg.USE_NURBS:
        H = NURBSHeliostat.from_dict(cp, device, nurbs_config=cfg.NURBS, config= cfg.H)
    else:
...
def train_batch(opt, sched, H, ENV, R, targets, sun_origins, epoch, writer=None):
    ...
    for (i, (target, sun_origin)) in enumerate(zip(
        ...
        # Plot target images to Tensorboard
            if writer:
                if epoch % 50 == 0:
                    writer.add_image(f"target_{i}/prediction", utils.colorize(pred_bitmap), epoch)
            
        # Compare metrics
            with th.no_grad():
                H.align_reverse()
                num_missed += (R.indices.numel() - R.indices.count_nonzero()) / len(targets)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50 == 0 and cfg.SAVE_RESULTS:
            plotter.plot_surfaces(
                ...
                th.tile(th.tensor([0, 0, 1], device=device), (H_target._discrete_points_orig.shape[0], 1)),
        ...
        #Save Section
        if cfg.SAVE_RESULTS:
            if loss.detach().cpu() < best_result: #store to array
                ...
                # save_data['config'] = cfg.H
                found_new_best_result = True
            if epoch % 100 == 0 and found_new_best_result == True: #store to file
